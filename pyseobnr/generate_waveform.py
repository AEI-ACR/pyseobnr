...
def generate_prec_hpc_opt(
    ...
    polarizations directly summing over the co-precessing frame modes.
    ...
    Args:
        ...
        omega_ref (float, optional): Reference orbital frequency, in geometric
            units. Defaults to None, in which case equals omega_start
        settings (Dict[Any,Any], optional): Any additional settings to pass to
            model. Defaults to None
        debug (bool, optional): Run in debug mode and return the model object.
            Defaults to False

    Returns:
        Tuple: Either (time, polarization) or (time, polarizations, model)
            if debug=True
    """
...
def generate_modes_opt(
    ...
    Returns:
        Tuple: Either (time, dictionary of modes) or (time, dictionary of modes, model)
        if ``debug=True``
    """
    ...
    if approximant != "SEOBNRv5EHM" and (
        (eccentricity is not None and eccentricity != 0)
        or (rel_anomaly is not None and rel_anomaly != 0)
    ):
        raise ValueError(
            "The selected approximant does not support input values for the "
    ...
    # checks the anti-symmetric parameters
    if settings and settings.get("enable_antisymmetric_modes", False):
        if approximant != "SEOBNRv5PHM":
            raise ValueError(
                f"Antisymmetric modes not available for approximant {approximant}."
            )
    ...
    elif approximant == "SEOBNRv5EHM":
        if eccentricity is None:
            eccentricity = 0.0
        if rel_anomaly is None:
            rel_anomaly = 0.0
        if not 0.0 <= eccentricity < 1.0:
            raise ValueError(
                "The value of eccentricity must be inside the interval [0, 1)."
            )
        RR_force = SEOBNRv5RRForceEcc("Ecc")
        model = SEOBNRv5EHM.SEOBNRv5EHM_opt(
            q=q,
            chi_1=chi1_z,
            chi_2=chi2_z,
            omega_start=omega_start,
...
class GenerateWaveform:
    ...
    def __init__(self, parameters):
        ...
        bool antisymmetric_modes_hm:
            if set to ``True``, will enable the higher modes ``(3,3)`` and ``(4,4)`` for the
            anti-symmetric modes corrections. Overrides ``antisymmetric_modes``.
        ...
        bool gwsignal_environment:
            if set and evaluates to ``True``, indicates that the code is being run
    ...
    def _validate_parameters(self, parameters):
        ...
        if (
            not isinstance(parameters["mass1"], numbers.Real)
            or isinstance(parameters["mass1"], bool)
            or not isinstance(parameters["mass2"], numbers.Real)
            or isinstance(parameters["mass2"], bool)
        ):
            raise ValueError("Only 'float' and 'int' values of masses are supported.")
        ...
        if mass1 * mass2 / Mtot**2 < 100.0 / (1 + 100) ** 2:
            raise ValueError(
                "Internal function call failed: Input domain error. Model is "
                "only valid for systems with mass-ratio up to 100."
            )
        ...
        if parameters.get("enable_antisymmetric_modes", False):
            if parameters["approximant"] != "SEOBNRv5PHM":
                raise ValueError(
                    "Antisymmetric modes not available for approximant "
