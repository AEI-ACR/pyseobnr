...
def EOBCalculateNQCCoefficients_freeattach_BNS(
    amplitude,
    phase,
    r,
    pr,
    ...
    Also note that we are only fitting (a1, a2) in contrast to the BBH case.
    ...
    # Compute amplitude NQCs (a1,a2) by solving Eq.(10) of T1100433v2
    # Evaluate the Qs at the right time and build Q matrix (LHS)
    idx = np.argmin(np.abs(time - nrTimePeak))
    N = 5
    left = np.max((0, idx - N))
    ...
    Q[:, 0] = intrp_q1LM.derivatives(nrTimePeak)[0:2]
    Q[:, 1] = intrp_q2LM.derivatives(nrTimePeak)[0:2]

    # Build the RHS of Eq.(10)
    # Compute the NR fits
    nra = fits_dict["amp"]
    nraDot = fits_dict["damp"]
    ...
    # Assemble RHS
    amps = np.array([nra - amp, nraDot - damp])[0]

    # Solve the equation Q*coeffs = amps
    # res = np.linalg.solve(Q, amps)

    coeffs["a1"] = amps / Q[0, 0]  # res[0]
    coeffs["a2"] = 0.0  # res[1]
    coeffs["a3"] = 0.0

    if coeffs["a1"] > 2e4:
        coeffs["a1"] = 2e4
        fits_dict["amp"] = coeffs["a1"] * Q[0, 0]
    ...
    P = -intrp_p1.derivatives(nrTimePeak)[1:-1][0]
    ...
    # Compute the NR fits
    nromega = np.abs(fits_dict["omega"])
    nromegaDot = np.abs(fits_dict["domega"])
