...
from pyseobnr.auxiliary.sanity_checks.single_waveform_tests import (
    amplitude_hierarchy_test,
)
...
def mismatch_v5P_strain(
    ...
    time_v5, modes_v5, hp_td_v5, hc_td_v5 = generate_v5PHM_waveform(
        m1,
        m2,
        0.0,
        0.0,
    ...
    time_v5p, modes_v5p, hp_td_v5p, hc_td_v5p = generate_v5PHM_waveform(
        m1,
        m2,
        0.0,
        0.0,
...
def process_one_case(input):
    ...
    m1, m2, chi1, chi2, iota, approx = input

    m1, m2, chi1, chi2, iota, mm = mismatch_v5P_strain(m1, m2, chi1, chi2, iota, approx)
...
if __name__ == "__main__":
    ...
    p.add_argument("--approx", type=str, help="Approx", default="SEOBNRv5PHM")
    ...
    approx_arr = np.repeat(args.approx, N)

    lst = [
        (m1_msun, m2_msun, s1z, s2z, iota, approx)
        for m1_msun, m2_msun, s1z, s2z, iota, approx in zip(
            m1, m2, chi1arr, chi2arr, iota_arr, approx_arr
        )
    ]
    ...
    if args.plots:
        ...
        plt.tight_layout()
        plt.savefig(
            f"{plt_dir}/mm_hist_v5PHM_align_strain.png",
            bbox_inches="tight",
            dpi=300,
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            f"{plt_dir}/mm_cdf_v5PHM_align_limit_strain.png",
            bbox_inches="tight",
            dpi=300,
        )
        plt.close()
